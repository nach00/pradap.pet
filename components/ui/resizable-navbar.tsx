		...
		>
			{items.map((item, idx) => (
				<a
					...
					onClick={onItemClick}
					className={cn(
						"relative px-4 py-2 text-neutral-600 transition-colors dark:text-neutral-300",
						item.isActive && "text-neutral-900 font-semibold dark:text-white",
					)}
					key={`link-${idx}`}
					href={item.link}
				>
...
export const MobileNav = ({ children, className, visible }: MobileNavProps) => {
	return (
		<motion.div
			...
			className={cn(
				"relative z-50 mx-auto w-full max-w-[calc(100vw-2rem)] flex-col items-center justify-between bg-transparent px-0 py-2 hidden",
				visible && "bg-white/80 dark:bg-neutral-950/80",
				className,
			)}
		>
